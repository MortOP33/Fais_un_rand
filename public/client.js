...
function createPageJeuMaitre() {
  ...
  pageJeuMaitre.innerHTML = `
    ...
    <div id="jeuCadres" style="width:100%;display:flex;flex-direction:row;justify-content:space-between;align-items:stretch;margin-bottom:24px;max-width:700px;">
      <div id="jeuReponseCadre" style="display:none; min-width:220px; max-width:220px;align-self:stretch;"></div>
      <div id="jeuComplementCadre" style="display:none; flex:1;align-self:stretch;"></div>
...
socket.on('afficher_question', ({ question, index, total, joueurs, themeImages }) => {
  ...
  // Cadre réponse et complément cachés par défaut
  document.getElementById('jeuReponseCadre').style.display = "none";
  document.getElementById('jeuComplementCadre').style.display = "none";

  // Cadre réponse aligné sur colonne "Joueur", complément occupe le reste
  document.getElementById('jeuReponseCadre').innerHTML = `<div style="background:#2b2c32;border-radius:16px;padding:18px 38px;display:flex;align-items:center;justify-content:center;font-size:2.2em;font-weight:bold;min-width:220px;max-width:220px;text-align:center;box-shadow:0 2px 13px #0005;border:4px solid #3855d6;color:#fff;height:100%;">${roundAnswer}</div>`;
  document.getElementById('jeuComplementCadre').innerHTML = `<div style="font-size:1.18em;font-weight:500;display:flex;align-items:center;height:100%;color:#ececec; margin-left:36px; width:100%; max-width:650px;">${roundComplement}</div>`;
...
// ----------- PAGE JOUEUR : champ de saisie sous avatar -----------
socket.on('afficher_saisie_joueur', ({ code }) => {
  if (codeInput.value.trim().toUpperCase() === code && joueurAvatar && joueurPseudo) {
    avatarsContainer.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; margin-top:24px;">
        <div style="display:flex; align-items:center; justify-content:center; gap:18px;">
          <img src="${joueurAvatar}" class="avatar-maitre" alt="" />
          <span class="player-name">${joueurPseudo}</span>
        </div>
        <div id="reponseForm" style="margin-top:24px; display:flex; flex-direction:column; align-items:center; gap:12px; width:100%;">
          ...
          <div id="joueurErreur" style="color:#d23c3c;font-size:1em;margin-top:2px;"></div>
        </div>
      </div>
    `;
    document.getElementById('champReponseJoueur').value = joueurResponse || "";
    document.getElementById('btnEnvoyerReponse').onclick = function() {
      ...
      if (!/^[-+]?\d*\.?\d+$/.test(val)) {
        document.getElementById('joueurErreur').textContent = "Réponse numérique attendue.";
        return;
      }
      document.getElementById('joueurErreur').textContent = "";
      joueurResponse = val;
